...
class Quest3ArmInfoTransformer:
    ...
    def compute_hand_pose(self, side):
        ...
        # body_yaw 는 회전벡터 성분(axis[1])이 아니라 오일러 각으로 뽑아야 한다.
        # matrix_to_axis_angle 의 angle 은 2*atan2(|xyz|, w) 로 항상 >=0 이라
        # 음의 회전을 축 부호로 표현하고, 그 부호가 튀면 허리가 반대로 돈다.
        # 또 rotvec 성분은 각도가 아니어서 pitch/roll 이 섞이면 값이 왜곡된다.
        # 인덱스 1 = y축 회전 = 이 VR 프레임의 yaw
        #   (같은 규약: body_pitch=[0], body_roll=[2], monitor_quest3 의 head pitch=[0]/yaw=[1])
        # NOTE: chest_axis_agl 은 팔/손 좌표변환에도 쓰이므로 위 줄은 건드리지 않는다.
        # 부호 반전: VR 프레임(Y-up)의 y축 회전 방향이 ROS(Z-up, 반시계=+)와 반대다.
        # 반전 전에는 허리가 사람과 반대로 돌았고, 조작자가 보정하려 할수록
        # 오차가 커지는 양의 되먹임이 되어 떨림 -> 발산 -> 컨트롤러 트립으로 이어졌다.
        self.head_body_pose.body_yaw = -chest_rpy[1]
        self._yaw_dbg = getattr(self, "_yaw_dbg", 0) + 1
        if self._yaw_dbg % 70 == 0:   # 약 1Hz. 검증 끝나면 지워도 됨
            print("[yaw] euler_y=%+7.1f -> body_yaw(sent)=%+7.1f deg"
                  % (chest_rpy[1]*57.2958, self.head_body_pose.body_yaw*57.2958))
